...
void COutputWnd::Output(LPCTSTR lpszItem)
{
	m_wndOutputBuild.AddString(lpszItem);
	int nCount = 0;
nCount = m_wndOutputBuild.GetCount(); // Declare m_LisBox as a control in SceneView member variables.
m_wndOutputBuild.SetCurSel(nCount - 1); // List box is zero based.

m_wndOutputBuild.SetTopIndex(nCount - 1); // moves the view.
}
